var ExportView = (function () {
    ...
    var triggerControls = function () {
        $('#btnCreate').click(function () {
            ExportView.voidAdd();
        }); // onClick to load addNew items
        $('#btnRefreshTable').click(function () {
            ExportView.refreshTable();
        }); // onClick to load refresh datatable
        $('#btnSave').click(function () {
            ExportView.voidSave();
        }); // onClick to load for save database
    }
    toastr.options = {
        "closeButton": true,
        "debug": false,
    ...
    return {
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        "mRender": function (data, type, full) {
                            return "<button type='button' class='btn btn-rounded btn-outline btn-info' href='javascript:void(0)' title='Edit' onclick='ExportView.voidEdit(" + '"' + data.catalogID + '"' + ")'><i class='fa fa-pencil'></i></button>" +
                                "<button type='button' class='btn btn-rounded btn-outline btn-danger' href='javascript:void(0)' title='Delete' onclick='ExportView.voidRemoveById(" + '"' + data.catalogID + '"' + ")'><i class='fa fa-trash'></i></button>";
                        }
                    }
                ],
                buttons: [{
        ...
        voidSave: function () {
            ...
            var url;
            if (save_method === 'add') {
                url = "/Catalog/addEntity/";
            } else {
                url = "/Catalog/updateEntity/";
            }
        ...
        voidEdit: function (id) {
            ...
            $.ajax({
                url: "/Catalog/getById/" + id,
                type: "GET",
                dataType: "JSON",
                success: function (data) {
                    $('[name="CatalogID"]').val(data.catalogID);
                    $('[name="CatalogName"]').val(data.catalogName);
        ...
        voidRemoveById: function (id) {
            swal({
                    title: "Are you sure?",
                    text: "Your will not be able to recover this " + id + "file!",
                    type: "warning",
                    ...
                    if (isConfirm) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                if (data.statusCode === 200) //if success close modal and reload ajax table
                                {
                                    swal("Deleted!", "Your record " + id + " has been deleted.", "success");
                                    //toastr.success(data.status, "Thông báo!");
